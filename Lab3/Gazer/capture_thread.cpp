...
void CaptureThread::run() {
    ...
    while(running) {
        ...
        int msec = curTime.msecsTo(QTime::currentTime());
        if(msec > 1000) {
            curTime = QTime::currentTime();
            fps = frames / (msec / 1000.0);
            frames = 0;

            cv::Scalar openCvMean, openCvStdDev;
            cv::meanStdDev(tmp_frame, openCvMean, openCvStdDev);

            getMeanAndStdDev(tmp_frame);

            qRegisterMetaType <cv::Scalar>("cv::Scalar");
            emit showState(fps, openCvMean, openCvStdDev, mean, stdDev);
...
void::CaptureThread::getMeanAndStdDev(cv::Mat &frame){
    ...
    QVector<double> temp_mean (3, 0);
    QVector<double> temp_mean_pow (3, 0);
    for (int i = 0; i < frame.rows; i++){
        for(int j = 0; j < frame.cols; j++){
            for (int k = 0; k < 3; k++){
                temp_mean[k] += frame.at<cv::Vec3b>(i, j)[k];
                temp_mean_pow[k] += frame.at<cv::Vec3b>(i, j)[k] * frame.at<cv::Vec3b>(i, j)[k];
            }
        }
    }
    for (int i = 0; i < 3; i++){
        mean[i] = temp_mean[i] / (frame.rows * frame.cols);
        stdDev[i] = std::sqrt(temp_mean_pow[i] / (frame.rows * frame.cols) - mean[i] * mean[i]);
    }
