...
void MainWindow::openCamera()
{
    if(capturer != nullptr) {
        ...
        disconnect(capturer, &CaptureThread::showState, this, &MainWindow::updateState);
    ...
    connect(capturer, &CaptureThread::showState, this, &MainWindow::updateState);
    capturer->start();
...
void MainWindow::updateState(float fps, cv::Scalar openCvMean, cv::Scalar openCvStdDev, cv::Scalar mean, cv::Scalar stdDev){
    mainStatusLabel->setText(QString("FPS: %1 , OpenCvMean: (%2, %3, %4), OpenCvStdDev: (%5, %6, %7) \n Mean: (%8, %9, %10), StdDev: (%11, %12, %13)")
                             .arg(int(fps))
                             .arg(int(openCvMean[0])).arg(int(openCvMean[1])).arg(int(openCvMean[2]))
                             .arg(int(openCvStdDev[0])).arg(int(openCvStdDev[1])).arg(int(openCvStdDev[2]))
                             .arg(int(mean[0])).arg(int(mean[1])).arg(int(mean[2]))
                             .arg(int(stdDev[0])).arg(int(stdDev[1])).arg(int(stdDev[2]))
            );
}
